...
const templatingWithMustache = () => {
  ...
  characters.forEach(Object => {
    char.push(new Character(Object));
  });
...
const getCourseKeys = (obj) => {
  const values = Object.keys(courseInfo);
  return values;
};
...
const getHouses = (arr) => {
  ...
  for (const key in characters) {
    houses.push(characters[key].house);
  }
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  for (const key in characters) {
    if (characters[key].name === character) {
      const child = Object.values(characters[key].children);
      if (child.length > 1) {
        return true;
      } else {
        return false;
