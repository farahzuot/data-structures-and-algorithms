...
const validatePin = (pin) => {
  let pattern = /\b[0-9][0-9][0-9][0-9]\b/g;
  let pat2 = pattern.test(pin);
  return pat2;
};
...
const validateEmail = (email) => {
  let pattern = /^[\w-\.]+[a-z]+@([\w-]+\.)+[\w-]{3,4}$/g;
  let pat2 = pattern.test(email);
  return pat2;
};
...
const validatePhoneNumber = (phoneNumber) => {
  let pat = /^(\(\d{3}\)|\d{3})[- ]?\d{3}[- ]?\d{4}$/;
  let pat2 = pat.test(phoneNumber);
  return pat2;
};
...
describe('Testing challenge 3', () => {
  ...
  test('It should fail things that aren\'t email addresses', () => {
    ...
    expect(validateEmail('middle.names@foryou.com')).toBeFalsy();
  });
});
