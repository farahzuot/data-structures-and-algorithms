...
const createServer = () => {
  ...
  app.get('/aboutme', (req, res) => {
    res.status(200).send('my name is farah alzuot, and i am a 23-years old.');
  });
  app.get('/favoritefoods', (req, res) => {
    let favFood = ['pizza', 'shawerma', 'burger'];
...
const divisibleByFiveTwoToThePower = (input) => {
  let arr = input.map(value => {
    //console.log(value)
    let arr2 = value.map(value2 => {
      //console.log(value2%5===0);
      if ((!(isNaN(value2))) && (value2 % 5 === 0) &&(!(typeof value2 ==='string'))) {
        //console.log(value2);
        return Math.pow(2, value2);
      }
    });
    return arr2;
  });
